...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function exportarArchivo2(formato) {
        var fecInicio = document.querySelector('input[name="fecha_inicio2"]').value;

        var url = "{{ route('exportar.diferencia.consumos') }}?";

        if (fecInicio) {
            url += "fecha_inicio2=" + encodeURIComponent(fecInicio) + "&";
        }
